...
## Step 4: Install Browser-Use Integration (Optional)

If you're using browser-use, install the optional dependency:

```bash
pip install auth-agent-sdk[browser-use]
```

This installs both the SDK and browser-use integration tools.

## Step 5: Import Auth Agent Tools

The `AuthAgentTools` class is included in the SDK package. Import it directly:
...
<Note>
**Alternative import** (available in future SDK versions):
```python
from auth_agent_sdk.agent import AuthAgentTools
```

If you get an `ImportError`, make sure you installed the browser-use extra:
`pip install auth-agent-sdk[browser-use]`
</Note>
